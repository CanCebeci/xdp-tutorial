...
/* Can: we are moving the BTF maps into the "maps" section.
 * 1 - We don't know what they are supposed to do but assume they are not crucial
 * 2 - My kernel does not have BTF and if there is anything at all in .maps
 * 	   libbpf will decide that BTF is crucial and won't load.
 * 
 * All map_def struct in "maps" must have the same size. 
 */
// struct {
...
struct bpf_map_def SEC("maps") LB4_REVERSE_NAT_SK_MAP = {
    .type = BPF_MAP_TYPE_LRU_HASH,
    .key_size = sizeof(struct ipv4_revnat_tuple),
    .value_size = sizeof(struct ipv4_revnat_entry),
    .max_entries = LB4_REVERSE_NAT_SK_MAP_SIZE,
};
...
__section("secname1") //__section("cgroup/connect4")
...
__section("secname2") //__section("cgroup/post_bind4")
...
__section("secname3") //__section("cgroup/bind4")
...
__section("secname4") //__section("cgroup/sendmsg4")
...
__section("secname5") //__section("cgroup/recvmsg4")
...
__section("secname6") //__section("cgroup/getpeername4")
...
struct bpf_map_def SEC("maps") LB6_REVERSE_NAT_SK_MAP = {
    .type = BPF_MAP_TYPE_LRU_HASH,
    .key_size = sizeof(struct ipv6_revnat_tuple),
    .value_size = sizeof(struct ipv6_revnat_entry),
    .max_entries = LB6_REVERSE_NAT_SK_MAP_SIZE,
};
...
__section("secname7") //__section("cgroup/post_bind6")
...
__section("secname8") //__section("cgroup/bind6")
...
__section("secname9") //__section("cgroup/connect6")
...
__section("secname10") //__section("cgroup/sendmsg6")
...
__section("secname11") //__section("cgroup/recvmsg6")
...
__section("secname12") //__section("cgroup/getpeername6")
