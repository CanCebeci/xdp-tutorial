					  ...
					  const char *obj_name)
{
	...
	//obj->efile.maps_shndx = {0};
	obj->efile.len_maps_shndx = 0;
...
static int bpf_object__init_user_maps(struct bpf_object *obj, bool strict)
{
	...
	if (obj->efile.len_maps_shndx == 0)
		return 0;

	if (!symbols)
		return -EINVAL;
	pr_warn("---> number of map sections: %d\n", obj->efile.len_maps_shndx);
	for (int j = 0; j < obj->efile.len_maps_shndx; j++) {
			int i, map_def_sz = 0, nr_maps = 0, nr_syms;
		scn = elf_getscn(obj->efile.elf, obj->efile.maps_shndx[j]);
		if (scn)
			data = elf_getdata(scn, NULL);
		if (!scn || !data) {
			pr_warn("failed to get Elf_Data from map section %d\n",
				obj->efile.maps_shndx[j]);
			return -EINVAL;
		}
		...
		*/
		...
		pr_warn("---> number of symbols: %d\n", nr_syms);
		for (i = 0; i < nr_syms; i++) {
			GElf_Sym sym;

			if (!gelf_getsym(symbols, i, &sym))
				continue;
			if (sym.st_shndx != obj->efile.maps_shndx[j])
				continue;
			nr_maps++;
		}
		pr_warn("---> number of maps: %d\n", nr_maps);
		//nr_maps = 1;
		...
		for (i = 0; i < nr_syms; i++) {
			...
			if (!gelf_getsym(symbols, i, &sym))		// CAN: change i to j
				continue;
			if (sym.st_shndx != obj->efile.maps_shndx[j])
				continue;
				   ...
				   const char *pin_root_path)
{
	...
	while ((scn = elf_nextscn(elf, scn)) != NULL) {
		...
		if (strcmp(name, "license") == 0) {
			...
			obj->efile.maps_shndx[obj->efile.len_maps_shndx] = idx;
			obj->efile.len_maps_shndx++;
...
static bool bpf_object__shndx_is_maps(const struct bpf_object *obj,
				      int shndx)
{
	for (int i = 0; i < obj->efile.len_maps_shndx; i++) {
		if (shndx == obj->efile.maps_shndx[i])
			return true;
	}
...
bpf_object__create_maps(struct bpf_object *obj)
{
	...
	pr_warn("---> in __create_maps: obj->nr-maps = %ld\n", obj->nr_maps);
			...
			struct bpf_object **pobj, int *prog_fd, char *outer_map_names[], int num_outer_maps)
{
	...
	for (int i = 0; i < num_outer_maps; i++) {
		...
		pr_warn("Found outer map with name %s: %s\n", outer_map_names[i], outer_map->name);
		if (outer_map == NULL) {
		...
		int inner_map_fd = bpf_create_map(
			...
			sizeof(__u32) * 32749, // value_size      #define LB_MAGLEV_LUT_SIZE 32749
			1, // max_entries
			0); // flag
